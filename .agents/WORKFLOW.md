...
- Go-backed Symphony npm commands must run through the repo-local wrapper that sets repo-local `GOCACHE`, repo-local `GOMODCACHE`, and `GOFLAGS=-modcacherw`, then normalizes the local module cache back to user-writable permissions after the Go command exits. The repo-local caches keep daemon/test runs isolated from host-global Go state, and the post-run permission normalization prevents future module downloads from becoming read-only teardown blockers in Symphony worktrees.
