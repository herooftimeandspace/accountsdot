...
pull_requests:
  ...
  materialize_post_merge_codex_review_threads: true
  post_merge_review_window_hours: 168
  post_merge_review_pr_limit: 50
verification:
  docs_only:
    - markdown review
  go_behavior:
...
- Codex Review is complete only when the newest review request has a later explicit clean signal from the configured Codex Review actor. The implemented clean signals are a `THUMBS_UP`/`+1` reaction from `chatgpt-codex-connector[bot]` on the main PR conversation or an `@codex` review request comment, or a later top-level PR comment from a configured `codex_review_authors` actor containing one of the configured `codex_review_success_comments` phrases such as "all PR issues have been addressed", "merge is clean", or "no actionable feedback". When all other merge gates pass, the PR should move toward automatic clean merge instead of waiting for manual confirmation.
...
- Current unresolved Codex Review threads on a merged PR cannot be remediated on the closed PR branch. The Synchronizer inspects recent merged Phase 0 PRs within the configured `post_merge_review_window_hours` and `post_merge_review_pr_limit`, then creates or updates one deduplicated `agent-ready` follow-up issue per actionable Codex Review thread. Each issue preserves the source PR, merge commit, thread id, thread URL, file path, line, severity, acceptance criteria, implementation notes, and verification expectations. These issues stay eligible for normal Symphony dispatch unless they carry a human-only, security, production-write, or other blocking label.
...
- Go-backed Symphony npm commands must run through the repo-local wrapper that forces repo-local `GOCACHE`, repo-local `GOMODCACHE`, and `GOFLAGS=-modcacherw`, then normalizes the local module cache back to user-writable permissions after the Go command exits. The wrapper must ignore inherited `GOCACHE` and `GOMODCACHE` values so daemon/test runs never mutate host-global Go state, and the post-run permission normalization prevents future module downloads from becoming read-only teardown blockers in Symphony worktrees.
...
- The daemon writes an initial live snapshot immediately after acquiring `daemon.lock`. Status and TUI readers must reconcile stale `controller.json`/`status.json` files against a newer live lock before displaying lifecycle or PID, because a prior failed daemon snapshot must not hide the currently running daemon while the first tick is still pending or blocked.
...
- If there is no Codex Review response yet, a thumbs-up reaction from `chatgpt-codex-connector[bot]` on the PR conversation or review-request comment is an implemented explicit clean signal. Later top-level Codex Review comments that explicitly say all PR issues have been addressed, the merge is clean, or no actionable feedback remains are also implemented clean signals when the phrase is configured in `pull_requests.codex_review_success_comments` and the author matches `pull_requests.codex_review_authors`. In the implemented path, the PR is safe for merge when the reaction-based or comment-based clean signal, other merge gates, and current thread-aware review state are clear.
...
When Codex Review leaves current actionable feedback after a PR has already merged, the Synchronizer cannot safely push fixes back to that merged PR. The automated merged-PR intake path inspects recent merged Phase 0 PRs, filters current unresolved Codex Review threads from configured review authors, and materializes each actionable thread as a deduplicated GitHub issue with `agent-ready`, `bug`, the relevant phase label, source PR, merge commit, thread id, thread URL, file path, line, severity, acceptance criteria, implementation notes, and verification expectations. These issues are ordinary runnable work unless another configured blocking label or safety rule applies.
