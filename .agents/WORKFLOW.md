...
- The daemon writes an initial live snapshot immediately after acquiring `daemon.lock`. Status and TUI readers must reconcile stale `controller.json`/`status.json` files against a newer, fresh live lock before displaying lifecycle or PID, because a prior failed daemon snapshot must not hide the currently running daemon while the first tick is still pending or blocked. A lock older than the daemon lock maximum is not live status evidence even when its recorded PID exists, because PID reuse by an unrelated process must not project a dead daemon as healthy.
