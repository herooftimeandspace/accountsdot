...
If a PR does not have a prepared issue workspace but its branch uses an approved automation prefix, the runner may create a dedicated PR remediation workspace under `dispatch.workspace_root` from the remote PR branch. This is intended for manual or out-of-band PRs that still need Codex Review, draft, or merge-conflict remediation. The runner must still refuse unsafe branch names, missing remote branches, dirty worktrees, or branches already checked out elsewhere.
