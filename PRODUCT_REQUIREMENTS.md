...
- In the pre-phase 0 DEV slice, the Offboarding page exposes two HR/IT-only action buttons:
  ...
  - `Offboard Contractor`
- The two top-level Offboarding action buttons must render as one right-side vertical action group, not side by side, so the related HR/IT manual workflows remain visually connected to the Offboarding dashboard header/action area without leaving excess horizontal whitespace.
