...
- The system generates district email candidates; operators do not enter or override the district email in this drawer. The approved generated domain for new manual Non-Escape contractor accounts is `@ext.wusd.org`; the prior/current contractor pattern using `@wusd.org` is not approved for newly generated contractor addresses. Existing active contractor addresses are preserved unless a separate migration/remediation workflow is explicitly approved. Candidate order is `[f][lastname]@ext.wusd.org`, `[firstname].[lastname]@ext.wusd.org`, `[f].[lastname]@ext.wusd.org`, then `[f][lastname][nn]@ext.wusd.org` with a zero-padded numeric suffix until unique.
