...
- Scheduled Emergency Offboarding requires a future effective date and time. The submitted payload must include the selected person, scheduled execution mode, and effective timestamp; the server must reject missing, malformed, or past effective timestamps before any mock state is recorded.
