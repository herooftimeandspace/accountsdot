...
| Site assignment | primary site, site scope, district-wide vs site-scoped eligibility | Escape, Aeries staff/teacher/scheduling, Google/SAML site scope | HR temporary site override, HR/IT InformedK12-backed primary-site selection, IT Admin permission/site mapping | Provider-owned site wins by default. InformedK12 form values are evidence for a reviewed dashboard decision, not direct provider-owned site authority. A documented temporary override or InformedK12-backed dashboard selection may remain effective only until the authoritative upstream value matches the intended correction, a newer authoritative assignment safely supersedes the planning context, a newer reviewed form-supported decision replaces it, the value is manually reconciled, or a different upstream value requires conflict review. |
...
- An InformedK12-backed primary-site selection is a dashboard-managed site-assignment decision for an Escape-backed employee whose Escape site is incomplete, conflicting, stale, or not yet corrected. The selection may use a reviewed active InformedK12 attachment as evidence, but it must keep raw Escape site values, raw InformedK12 form values, parsed site signal, and active dashboard selection as separate facts. It has no fixed automatic expiration; it requires a review-after timestamp for HR visibility and persists until current Escape data unambiguously matches the selected site, a newer reviewed form-supported decision replaces it, a newer active Escape job assignment confirms the same selected site under the new context, or HR/IT manually reconciles it with an audit reason. Aeries, Google, SAML, and other non-Escape site signals may support review, but they cannot automatically clear the selection for an Escape-backed employee. Later Escape disagreement marks a conflict and blocks dependent write-capable planning instead of silently overwriting the selection or Escape source values.
...
- `informedk12-primary-site-selection-cleared`: an active InformedK12-backed primary-site selection is cleared or superseded when current Escape data later unambiguously matches the selected site, a newer active Escape job assignment confirms the same selected site under the new context, or a newer reviewed form-supported decision replaces it, with audit evidence
