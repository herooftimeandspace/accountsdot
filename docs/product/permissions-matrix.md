...
| `/api/v1/dev/offboarding/candidates` | IT Admin, HR | `401` signed out, `403` non-HR/non-IT before candidate data is returned | Exposes employee IDs and active contractor search corpus only to HR/IT; HR search omits IT Admin targets | Implemented | Manual offboarding action tests |
| `/api/v1/dev/offboarding/emergency-deprovision` | IT Admin, HR | `401` signed out, `403` non-HR/non-IT before mutation handling; HR-to-IT-Admin target requests return `403` before mock state is created | In-memory DEV mock schedule only; supports immediate mode and scheduled future effective date/time; no provider write | Implemented | Manual offboarding action tests |
| `/api/v1/dev/offboarding/contractor-offboarding` | IT Admin, HR | `401` signed out, `403` non-HR/non-IT before mutation handling; HR-to-IT-Admin target requests return `403` before mock state is created | Contractor-specific shortcut; requires explicit selected contractor and valid `YYYY-MM-DD` date; no provider write | Implemented | Manual offboarding action tests |
...
| `/api/v1/dev/pages/reports/zoom-desk-phone-renames` | IT Admin | `401` signed out, `403` non-IT | Shows only pending manual adjustment and error Zoom desk phone rename rows with IncidentIQ asset links; healthy/completed/non-actionable rows are excluded | Implemented | Zoom Desk Phone Renames report tests |
...
| `/api/v1/dev/pages/room-moves`, `/room-moves/bulk-draft` | IT Admin, Site Admin, Site Secretary | Requires route and site/draft scope | IT Admin can manage district/inter-site rows; site-scoped users can view all assigned-site rows, including IT-authored rows, but receive read-only edit flags unless they authored the draft | Implemented | Room Moves page and draft tests |
| `/api/v1/dev/room-moves/*` | IT Admin, Site Admin, Site Secretary according to draft scope and author ownership | Mutations enforce route, site scope, author ownership for site-scoped users, and admin-only revert authority | DEV mock room-move planning only | Implemented | Room Moves mutation tests |
