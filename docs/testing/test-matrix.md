...
- Bucket-level evidence requirements for this phase:
  ...
  - `0E` health checks, metrics, and promotion plumbing
    ...
    - OpenAPI drift evidence proving generated client-facing API docs match the
      registered `/api/v1` route catalog and distinguish DEV mock surfaces from
      planned or callable DB-backed APIs
- `0A` repo-local safety artifacts and environment playbooks
  ...
  | `P0-0A-002` | Environment Role Separation | Run `npm run environment-roles:check` to verify the checked-in deploy examples distinguish `dev`, `staging`, and `main` roles, keep dev mock-backed, and block production-only assumptions in dev. | Run `npm run environment-roles:check` and confirm staging config is distinct from dev/main with `ENVIRONMENT_DATA_MODE=masked-read-only` and `ENVIRONMENT_DATA_SOURCE=masked-production-derived`. |
- `0B` core database schema, workflow engine, and recovery primitives
  | Scenario ID | Scenario Name | Dev Mock Verification | Staging Verification |
  | --- | --- | --- | --- |
  ...
  | `P0-0B-002` | Global Tick Ordering Integrity | Run `go test ./internal/db` to create ordered job and outbox-event mock rows and verify claim, recovery, and outbox-read SQL uses `global_tick`, never UUID, id, timestamp, or payload ordering. | Confirm staging job/event ordering matches dev expectations under concurrent inserts by recording inserted `global_tick` values and selected order in the external evidence ticket. |
  | `P0-0B-003` | Overlap Protection Prevents Duplicate Job Family Runs | Simulate a second scheduled run starting while the first is still active and verify the second run is deferred without clobbering work; focused unit coverage lives in `internal/db/jobs_test.go` around `StartScheduledWorkflowRun`. | Verify staging prevents duplicate scheduled family execution and records overlap state, including the active run id, deferred run id, `deferred_from_run_id`, `overlap_state`, and `overlap_count`. |
- `0C` auth gate, breakglass, global pause, and emergency controls
  | Scenario ID | Scenario Name | Dev Mock Verification | Staging Verification |
  | --- | --- | --- | --- |
  | `P0-0C-001` | Staff Domain Allowlist Gate | Run `go test ./internal/auth ./internal/config -run 'TestP000C001'` to verify allowed `@wusd.org`, `@it.wusd.org`, and `@staff.wusd.org` identities can reach role mapping, while unauthorized domains are blocked before role logic. | Run the same `go test ./internal/auth ./internal/config -run 'TestP000C001'` check from the staging revision, and confirm staging configuration feeds the same default staff-domain allow/deny policy unless an approved deployment override is documented. |
  ...
  | `P0-0C-003` | Google Group And Attribute Role Mapping | Run `go test ./internal/auth -run 'TestP000C003'` to verify a staff-domain identity receives only roles derived from current Google group and SAML attribute inputs, authorized routes are allowed, unauthorized same-URL route checks return access denied, and a known staff identity with no role mapping receives access denied instead of partial content. | Confirm staging uses the approved Google Workspace groups/attributes and denies same-URL access for users without matching role assignments. |
  | `P0-0C-004` | Site Scope Recalculation | Verify site scope is recalculated from current group/attribute mapping input when a user's group or site assignment changes, when the admin-approved mapping JSON changes, and when a site assignment disappears. | Confirm staging reflects changed Google group/site assignments without stale cross-site access. |
  ...
  | `P0-0C-006` | IT Admin Auth Settings Preview | Run `go test ./internal/auth ./internal/web -run 'TestAdminAuthSettings\|TestEvaluateGoogleIdentityMapsGroupsAttributesAndSites'` to verify group, OU, and attribute mappings can be stored, audited, previewed, and kept IT Admin-only without enabling production login. | Confirm staging keeps production SAML/session issuance disabled for DB-backed mappings until a later production-login issue explicitly wires the previewed mappings into live auth. |
  | `P0-0C-006` | Global Pause Stops New Claims | Enable `system_controls.global_pause`, verify `internal/db.ClaimNextJob` returns `ErrGlobalPauseActive` before mutating `jobs`, and confirm a lightweight UI or diagnostic read remains online. Focused unit coverage lives in `internal/db/jobs_test.go`. | Confirm staging pause can be used as an emergency cutoff without requiring deployment rollback by enabling the staging `global_pause` row, proving no new lease is claimed, confirming diagnostics remain reachable, and restoring the prior pause value after evidence capture. |
...
- `0D` provider configuration, read-only connectivity, and mock scaffolding
  | Scenario ID | Scenario Name | Dev Mock Verification | Staging Verification |
  | --- | --- | --- | --- |
  | `P0-0D-001` | Provider Readiness Mock Success Path | Run `go test ./internal/provider ./internal/config -run 'TestP000D001'` to verify provider clients initialize and pass readiness against mocks without real outbound probes or writes. | Confirm staging read-only/provider connectivity checks succeed with configured non-prod credentials and writeback disabled. |
  | `P0-0D-002` | Provider Readiness Failure Surfacing | Run `GOCACHE=$PWD/.gocache go test ./internal/provider ./internal/config ./internal/web ./cmd/provisioner -run 'TestP000D002\|TestProviderConfigurationDiagnostics\|TestHealthReadyFailsProviderReadiness\|TestNewServerReportsProviderReadinessFailure'` to simulate missing credential labels, bad URLs, and bad certificate files, then verify `/health/ready` fails closed with sanitized `provider_<name>` diagnostics while `/health/live` remains available. | Confirm the same failures surface clearly in staging by disabling one `USE_MOCK_*` flag with a staging-safe missing credential, bad URL, or bad certificate drill; `/health/ready` must return `503` with actionable provider diagnostics while `/health/live` remains available. |
  | `P0-0D-003` | Aeries Previous-Year Staging Configuration | Run provider/config tests for `internal/provider.ResolveAeriesPreviousYearStagingConfig` and verify staging config resolves `current school year - 1` to `DatabaseYear=YYYY` while requiring masked previous-year read-only mode. | Confirm staging can successfully query masked previous-year Aeries data with the resolved `DatabaseYear=YYYY` query parameter, and record sanitized evidence without credentials, auth headers, certificates, or raw student/staff records. |
  | `P0-0D-004` | Provider Access Modes Classified Before Implementation | Verify the implementation plan provider capability classification matrix documents each provider as `batch-only source`, `projection-backed list/search`, `live detail read`, or `live write-path verification` where appropriate before code locks in a full-mirror design. | Confirm staging readiness review uses the same implementation-plan provider capability classification matrix and freshness expectations, and send any staging-discovered classification change back through the implementation plan before promotion. |
  | `P0-0D-005` | Auth Settings Provider Credentials And Default-Off Sync Controls | Run `go test ./internal/web -run TestAdminAuthSettingsPreviewAndCredentialsStaySanitized` to verify encrypted credential saves never return plaintext values, provider sync toggles default off and only mutate DB/audit state, and explicit Test actions record sanitized status without provider payload storage. | Confirm staging operators can save non-production encrypted credential fields, see only labels/fingerprints/status, run read-only tests on explicit click, and verify no imports, provider mutations, scheduled syncs, or production-login changes occur. |
- `0E` health checks, metrics, and promotion plumbing
  | Scenario ID | Scenario Name | Dev Mock Verification | Staging Verification |
  | --- | --- | --- | --- |
  | `P0-0E-001` | Readiness Fails Closed on Missing Dependency | Run `go test ./internal/web -run 'TestHealth(ReadyFailsClosedOnMissingRequiredDependency|ReadyFailsDependency|ReadyAllowsMissingOptionalSFTPCheck|Routes)$'` to simulate missing required DB, sequence, import-staging storage, and Google service-account checks, configured dependency failures, optional unwired SFTP, and `/health/live` staying `200 OK` while `/health/ready` returns `503 Service Unavailable`. | Confirm staging health endpoints behave consistently under dependency failure drills using staging-safe configuration, with sanitized evidence recorded outside the repo. |
  | `P0-0E-002` | Health Endpoints Reflect Pause and Dependency State | Run `go test ./internal/web ./cmd/provisioner -run 'TestHealth|TestMetrics|TestNewServerReportsInvalidHealthDatabaseConfig|TestHealthProbeErrorsAreBounded'` to verify `/health/live` remains online without DB-backed control checks during global pause, `/health/ready` and `/health` fail closed with `paused` or `degraded`, request cancellation reaches dependency probes, DB probe errors are bounded before they reach health state, and `/metrics` exposes `app_ready`, `app_global_pause`, and bounded dependency readiness gauges. | Confirm staging observability reflects the same readiness/pause semantics documented in `docs/operations/health-observability.md`, including `app_ready 0` while paused or degraded and no secret-bearing metric labels. |
  | `P0-0E-003` | Promotion Gate Requires Named Scenario Passes | Verify promotion tooling/checklist blocks advancement when required scenario evidence is missing, and verify `docs/operations/promotion-pipeline.md` maps local branch gates to the checked-in GitHub workflows. While the app is not stable enough for production promotion, use `python3 scripts/check_ci_promotion.py --release-prep` plus `python3 scripts/run_local_ci.py --target staging --dry-run` and `python3 scripts/run_local_ci.py --target main --dry-run` as the smallest non-merging proof. | Confirm staging promotion checklist enforces named scenario completion before main promotion and that the main promotion PR carries external runbook, IncidentIQ testing-ticket, and release/deployment metadata before merge. A placeholder-bearing `promote/staging-to-main` PR proves the blocker path only; do not merge it to `main` until production promotion is intentionally approved. |
  | `P0-0E-004` | OpenAPI Contract Distinguishes Callable And DEV Surfaces | Run `npm run openapi:check` and `go test ./internal/web -run 'TestOpenAPISpec|TestAppRoutes'` to verify `/api/v1/openapi.json` serves the generated contract, every registered `/api/v1` mux route is cataloged, every cataloged route has OpenAPI operation coverage, `x-wizard-surface` labels distinguish `dev-mock`, `accepted-no-op`, planned DB-backed, and callable runtime APIs, and generated DB-access/retry/idempotency extensions do not promote no-op or DEV mock mutations to durable writes. | Confirm the same generated `docs/api/openapi.json` is served by staging, remains drift-free against the checked-out route catalog, and does not label DEV-only mock endpoints as production/staging callable APIs or planned durable writes as already implemented. |
...
- `1B` onboarding/offboarding visibility and review-only mismatch queues
  ...
  | `P1-1B-004` | HR/IT Manual Offboarding Action Authorization | `HR / IT Admin` | Verify only HR and IT Admin can see or call Emergency Offboarding and Offboard Contractor candidate/search schedule APIs, that Site Admin, Site Secretary, Device Wrangler, Faculty and Staff, and signed-out requests receive denial before candidate data or mutation results are returned, and that HR candidate search does not expose actionable IT Admin targets. | Confirm staging preserves the same actor and target authorization boundary before any provider-backed offboarding candidate data or write planning is exposed. |
- `1C` phone directory visibility surfaces
  | Scenario ID | Scenario Name | Dev Mock Verification | Staging Verification |
  | --- | --- | --- | --- |
...
- Bucket-level evidence requirements for this phase:
  ...
  - `2G` InformedK12 form attachment and lifecycle evidence
    ...
    - primary-site selection evidence showing Escape values, InformedK12 values, and dashboard-selected values distinctly
...
- `2B` common-path onboarding and baseline update/deprovision lifecycle
  ...
  | `P2-2B-007` | DEV Emergency And Contractor Offboarding Scheduling | Verify Emergency Offboarding records an immediate DEV mock deprovision only after HR/IT selects an active employee or contractor, records a dated DEV mock deprovision only when scheduled mode includes a valid future effective timestamp with explicit timezone offset, rejects missing/malformed/timezone-less/past scheduled effective timestamps, and denies HR-originated immediate or scheduled termination of IT Admin targets before mock state is created. Verify Offboard Contractor remains a contractor-specific shortcut that records a dated DEV mock deprovision only after HR/IT selects an active manual contractor and clicks Schedule Offboarding, with HR-to-IT-Admin target denial enforced server-side. IT Admin actor behavior and normal HR-allowed non-IT targets must remain tested separately. | Confirm staging treats the workflow as what-if/pilot-gated and enforces the HR-to-IT-Admin target guard before any provider-backed offboarding mutation or write plan runs. |
  | `P2-2B-008` | Manual Employee Change Rollover Classification | Verify temporary HR site overrides, room/location overrides, manual Non-Escape assignments, InformedK12-backed site selections, permission/site-scope overrides, local employee attributes, and sync exception overrides receive the documented school-year lifecycle classification. | Confirm staging uses the same classification before building the HR rollover review queue or any rollover plan. |
  | `P2-2B-009` | HR Rollover Review Decision Audit | Verify HR `extend`, `remove`, and `reconciled` decisions validate required fields, require a new expiration/review date for extensions, reject unsupported removals, and produce audit history with actor, timestamp, reason, decision, and new expiration date when applicable. | Confirm staging persists the audit row before any local metadata cleanup or provider-affecting plan can proceed. |
  | `P2-2B-010` | Non-Reviewed Destructive Provider Removal Blocked | Verify rollover cleanup refuses provider-affecting removals unless HR reviewed the row, selected `remove`, what-if validation passed, and the Phase 2+ pilot allowlist check passed immediately before the mutation. | Confirm staging treats missing review, failed preview, or non-allowlisted targets as hard pre-write denials rather than warnings. |
  | `P2-2B-011` | Aeries School-Year Boundary Required For Rollover | Verify rollover setup reads the school-year boundary from Aeries School Info, collapses disagreement to earliest start and latest end, and stops with an operator-visible blocker when the boundary cannot be determined. | Confirm staging uses the documented Aeries-derived district boundary and does not fall back to a guessed school-year date. |
- `2C` reactivation and AD → Entra propagation warning handling
  | Scenario ID | Scenario Name | Dev Mock Verification | Staging Verification |
  | --- | --- | --- | --- |
  ...
  | `P2-2C-005` | Explicit Employee-To-Contractor Continuation Reuses Identity | Verify a former Escape employee rehired as a manual Non-Escape contractor reuses the existing identity only when HR/IT selects the former employee continuation link, exposes the `employee_contractor_continuation` decision, shows the employee/contractor timeline, records link and account-continuity audit events, and prevents duplicate generated identifiers. Confirm a matching former employee without the explicit link stays in `Needs Review` and cannot finalize. | Confirm staging preserves the same identity under the contractor/manual baseline only after explicit review, persists the audit trail, and does not perform provider writes unless the Phase 2+ what-if and pilot allowlist gates pass. |
  | `P2-2C-006` | Non-Adjacent Employee-To-Contractor Gap Warning | Verify the adjacent conversion threshold is `14` calendar days, linked conversions beyond that gap keep the account open only with explicit continuation approval, and the drawer timeline shows the gap warning with actor, timestamp, lifecycle owner, and bidirectional record links. | Confirm staging keeps longer-gap conversions reviewable and auditable rather than merging employee and contractor periods into one hidden status. |
  | `P2-2C-007` | Past-Dated Start Preserves Date And Schedules Catch-Up | Verify both Escape-backed and manual past-date starts preserve the source date, show the standard late-start warning, and schedule the next available workflow cycle. | Confirm staging treats past-date starts as late-but-valid work without rewriting the source/requested date. |
- `2D` preferred-name self-service and downstream sync
  | Scenario ID | Scenario Name | Dev Mock Verification | Staging Verification |
  | --- | --- | --- | --- |
  ...
  | `P2-2D-004` | Device Preference Self-Service Is Advisory And Audited | Verify My Profile renders the documented device-preference options, saves the current user's preference only, reloads the saved value, shows the annual school-year reminder text, and records actor/timestamp/before/after audit-shaped metadata without provider writes. | Confirm staging records the same self-service preference and audit metadata before any downstream provisioning workflow treats the value as an input. |
...
- `2G` InformedK12 form attachment and lifecycle evidence
  ...
  | `P2-2G-006` | Clear InformedK12 Site Signal | Verify a latest active attachment with one supported site-bearing field and one documented alias produces a `clear` signal with source form id, submitted timestamp, parsed site, confidence, raw value, and field reference. | Confirm staging shows the clear signal beside Escape data without writing to InformedK12, Escape, or dashboard override state. |
  | `P2-2G-007` | Missing InformedK12 Site Signal | Verify no active attachment or no retained supported site-bearing field produces a `missing` review state with no parsed site. | Confirm staging employee/contractor detail surfaces show the missing state as review context rather than a blank effective site. |
  | `P2-2G-008` | Ambiguous InformedK12 Site Signal | Verify supported fields that match no documented site alias or multiple different aliases produce an `ambiguous` review state while preserving all raw/source field values. | Confirm staging routes ambiguous signals to HR/IT review and does not auto-select a site. |
  | `P2-2G-009` | Stale InformedK12 Site Signal | Verify a single parsed signal older than the configured freshness window produces a `stale` review state while retaining the parsed site and source form timestamp. | Confirm staging freshness messaging matches the current InformedK12 reconciliation window before operators rely on the signal. |
  | `P2-2G-010` | InformedK12 Site Signal Conflicts With Escape | Verify a single parsed InformedK12 site that disagrees with current Escape site data produces a `conflicting` review state and keeps both source values distinct. | Confirm staging does not let a conflicting signal overwrite Escape or become an effective dashboard site without a separate audited workflow. |
  | `P2-2G-011` | Single Escape Site Wins Without Dashboard Selection | Verify an Escape-backed employee with one active resolved Escape site uses that Escape site as the active dashboard primary site without creating an InformedK12-backed selection, while any agreeing form evidence remains related evidence only. | Confirm staging preserves the same no-override behavior and shows Escape as the source of the active site. |
  | `P2-2G-012` | Conflicting Escape Sites Require HR/IT Review | Verify multiple active same-category Escape assignments with unresolved site disagreement produce an HR/IT review state, preserve every raw Escape site value, and do not silently pick or normalize a site. | Confirm staging routes the row to HR/IT review and blocks site-dependent write-capable planning until a documented decision exists. |
  | `P2-2G-013` | InformedK12-Backed Primary Site Selection Drives Planning | Verify HR or IT Admin can select a primary site from reviewed active InformedK12 evidence, with actor, timestamp, reason, related form id, before/after selection, Escape snapshot, InformedK12 snapshot, review-after timestamp, and audit event, and verify the planning preview shows Escape values, InformedK12 values, and active dashboard selection as distinct fields. | Confirm staging uses the audited dashboard selection only for app-level planning and does not write to Escape, InformedK12, Aeries, Google, SAML, or downstream providers as part of the selection. |
  | `P2-2G-014` | Primary Site Selection Clears Or Supersedes Safely | Verify a selection is cleared only when current Escape data later unambiguously matches the selected site for the Escape-backed employee, is superseded by a newer reviewed form-supported decision or a newer active Escape job assignment that confirms the same selected site under the new context, and becomes `Needs review` rather than silently changing when later Escape data disagrees with both the original snapshot and selected site. | Confirm staging preserves audit history for cleared, superseded, and conflict outcomes, treats Aeries/Google/SAML site signals as review evidence rather than automatic clear authority for this selection type, and blocks dependent write-capable planning while conflict remains unresolved. |
- `2E` actionable Google-active / Aeries-inactive controls
  | Scenario ID | Scenario Name | Dev Mock Verification | Staging Verification |
  | --- | --- | --- | --- |
...
- `3E` bulk summer rollover and recovery
  ...
  | `P3-3E-011` | Site Rollover Preloads Every Current Room Membership | Verify Site Rollover bulk drafts are populated from scoped-site room memberships rather than unique people, including one-room users, repeated-person multi-room users, classroom shared-line-only rows, relationship labels, site-scope filtering, and row preservation after bulk/default destination edits. | Confirm staging expands provider-backed current room and classroom shared-line data into one editable row per scoped-site membership without exposing department shared line groups or out-of-site rooms. |
