...
- `0E` health checks, metrics, and promotion plumbing
  ...
  | `P0-0E-004` | OpenAPI Contract Distinguishes Callable And DEV Surfaces | Run `npm run openapi:check` and `go test ./internal/web -run 'TestOpenAPISpec|TestAppRoutes'` to verify `/api/v1/openapi.json` serves the generated contract, every registered `/api/v1` mux route is cataloged, every cataloged route has OpenAPI operation coverage, and `x-wizard-surface` labels distinguish `dev-mock`, `accepted-no-op`, planned DB-backed, and callable runtime APIs. | Confirm the same generated `docs/api/openapi.json` is served by staging, remains drift-free against the checked-out route catalog, and does not label DEV-only mock endpoints as production/staging callable APIs. |
