...
- `2G` InformedK12 form attachment and lifecycle evidence
  ...
  | `P2-2G-014` | Primary Site Selection Clears Or Supersedes Safely | Verify a selection is cleared when upstream Escape/Aeries later unambiguously matches the selected site, is superseded by a newer reviewed form-supported decision or job assignment, and becomes `Needs review` rather than silently changing when later Escape data disagrees with both the original snapshot and selected site. | Confirm staging preserves audit history for cleared, superseded, and conflict outcomes and blocks dependent write-capable planning while conflict remains unresolved. |
- `2E` actionable Google-active / Aeries-inactive controls
  | Scenario ID | Scenario Name | Dev Mock Verification | Staging Verification |
  | --- | --- | --- | --- |
