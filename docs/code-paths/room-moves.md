...
- Updating an existing seeded single-move row creates an in-memory draft with that same draft id and suppresses the seed row, so the review table shows one edited row rather than the original row plus a duplicate.
...
- `frontend/src/pages/roomMovesModel.test.mjs` covers drawer-visible same-room validation, `Save and Apply` action labeling, removal of the side-drawer `Schedule` action, safe defaults, and close-state behavior.
