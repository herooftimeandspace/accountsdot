...
- Onboarding and offboarding dashboard behavior:
  ...
  - the DEV Emergency Offboarding drawer searches active employees and contractors by name, email, and employee ID, shows selected employment data, requires an explicit selected person in the submitted payload, and supports `immediate` and `scheduled` execution modes. Immediate mode records an in-memory `emergency_deprovision` mock action with actor, target, timestamp, `scheduled_for: immediate`, and `dev_mock_only` mode. Scheduled mode requires a future effective date/time and records an in-memory `emergency_scheduled_deprovision` mock action with actor, target, selected effective date/time, submission timestamp, status, and `dev_mock_only` mode.
