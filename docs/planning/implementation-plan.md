...
- The first checked-in runner lives at `scripts/symphony_runner.mjs`. It supports `npm run symphony:report`, `npm run symphony:ui-monitor`, and `npm run symphony:test`; its first production use case is the `ui-improvements` monitor that replaces the long heartbeat prompt with runner-owned queue reporting, lock handling, latest-code inspection, repo-skill routing, and Browser-evaluation handoff.
...
- Browser validation is delegated to the Codex automation wrapper. The runner emits `browser_evaluations[]`; the wrapper executes those requests with `@browser` and records `browser_results[]`. Missing Browser results leave the run in `needs_browser_evaluation`.
