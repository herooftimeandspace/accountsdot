...
- The shared left sidebar intentionally owns an independent viewport-bounded scroll offset when role-filtered navigation plus Platform Status and the DEV persona switcher exceed the available browser height. This preserves access to long IT Admin navigation and DEV mock-session controls on laptop-height viewports without moving the shared header, changing the inner page scroll container, widening the app frame, or disturbing right-drawer anchoring.
...
- Onboarding and offboarding dashboard behavior:
  ...
  - the DEV Emergency Offboarding drawer searches active employees and contractors by name, email, and employee ID, shows selected employment data, requires an explicit selected person in the submitted payload, and supports `immediate` and `scheduled` execution modes. Immediate mode records an in-memory `emergency_deprovision` mock action with actor, target, timestamp, `scheduled_for: immediate`, and `dev_mock_only` mode. Scheduled mode requires a future effective date/time with an explicit timezone offset and records an in-memory `emergency_scheduled_deprovision` mock action with actor, target, selected effective instant, submission timestamp, status, and `dev_mock_only` mode.
  - the DEV Offboard Contractor drawer remains a contractor-specific shortcut for manually created Non-Escape contractors: it searches active contractors only by name, email, and employee ID, shows selected contractor data, lets HR/IT edit the termination date locally, and records a dated in-memory `contractor_scheduled_deprovision` mock action only when `Schedule Offboarding` is submitted.
  - candidate search and scheduling endpoints must enforce the HR/IT manual-offboarding permission server-side before exposing employee IDs, contractor data, or mutation results; direct payload construction by other personas returns the same denied behavior as other protected APIs. HR-originated immediate emergency, scheduled emergency, and contractor scheduled termination requests against IT Admin targets must fail before any in-memory mock action, database row, workflow record, provider-operation plan, or external request is created. IT Admin actor behavior for IT Admin targets is tested separately and remains allowed in this DEV slice.
