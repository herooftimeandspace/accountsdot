...
- The first checked-in runner lives at `scripts/symphony_runner.mjs`. It supports `npm run symphony:report`, `npm run symphony:ui-monitor`, and `npm run symphony:test`; its first production use case is the `ui-improvements` monitor that replaces the long heartbeat prompt with runner-owned queue reporting, lock handling, latest-code inspection, conservative PR-branch rebase reconciliation, repo-skill routing, and Browser-evaluation handoff.
