...
| `/admin/auth-settings` | IT Admin | Admin child button visible only when child route is allowed | Signed-out `401`; non-IT `403` | None | `GET/POST/DELETE` `/api/v1/admin/auth-settings`; `POST` `/api/v1/admin/auth-settings/preview`; `PUT/PATCH/POST` `/api/v1/admin/external-sources/` | IT Admin-only auth mapping, validation preview, encrypted provider credential metadata, default-off sync toggles, and read-only credential tests. Production login and sync execution are not enabled by this page. |
...
| `/api/v1/admin/auth-settings` and `/api/v1/admin/auth-settings/*` | Lists, saves, deletes, and previews Auth Settings role and site-scope mappings. | IT Admin-only. Writes require a reason and write sanitized audit entries. Preview uses DB-backed mappings but does not create sessions, enable production SAML, or issue cookies. |
| `/api/v1/admin/external-sources/*` | Saves encrypted provider credential fields, toggles default-off source sync configuration, and records explicit read-only credential test status. | IT Admin-only. Writes require a reason and write sanitized audit entries. Credential values are encrypted at rest and never returned. Test actions validate saved credentials without starting imports, scheduling syncs, mutating providers, or storing provider payloads. |
