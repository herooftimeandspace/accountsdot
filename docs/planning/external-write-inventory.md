...
- `internal/db.ClaimNextJob` first reads `system_controls.global_pause` through `internal/db.IsGlobalPauseActive`. When the control row is enabled, the function returns `ErrGlobalPauseActive` before issuing any `jobs` mutation, so workers stop taking new leases while UI and diagnostic routes can remain online. When the pause row is missing or disabled, it updates one `jobs` row from `queued` to `running`, sets `lease_owner`, `lease_expires_at`, `lease_heartbeat_at`, and `updated_at`, and returns the claimed row ordered by `global_tick`. Worker callers must execute it inside `internal/db.WithRetry` so the pause read and claim participate in a `SERIALIZABLE` transaction and can be retried on serialization or deadlock errors.
