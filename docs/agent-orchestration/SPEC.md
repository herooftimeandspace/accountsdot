...
Go-backed Symphony package scripts must run through the repo-local wrapper that sets repo-local `GOCACHE`, repo-local `GOMODCACHE`, and `GOFLAGS=-modcacherw`, then normalizes the local module cache back to user-writable permissions after the Go command exits. The local caches keep daemon, test, TUI, status, and control commands from mutating host-global Go state, while the post-run permission normalization prevents generated-cache teardown and self-healing workspace cleanup from repeatedly failing on read-only module-cache directories.
