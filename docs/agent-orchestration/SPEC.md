...
Codex Review reconciliation is also conservative. The Phase 0 synchronizer and `ui-monitor` must read `reviewThreads` plus the implemented PR comment/reaction signal path so they can distinguish active, resolved, outdated, explicit clean, and pending review states in the fetched conversation window. They may automatically resolve only outdated unresolved threads authored by configured Codex Review accounts, because those comments no longer point at the current diff. When the synchronizer or monitor resolves any outdated thread, it must fetch review-thread state again before evaluating merge readiness so an obsolete conversation does not continue blocking a clean PR. Active Codex Review threads remain blocking review-remediation work until an agent applies an in-scope code or documentation fix, runs relevant verification, pushes the branch, and records the evidence needed to resolve or reply to the exact thread.
