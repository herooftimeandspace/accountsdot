...
- [docs/agent-orchestration/SPEC.md](docs/agent-orchestration/SPEC.md) defines the repo-local Symphony-style Codex orchestration contract for GitHub issue driven agent work. [.agents/WORKFLOW.md](.agents/WORKFLOW.md) is the runner-readable prompt and configuration contract for that workflow. The checked-in public runner is the Go CLI at `cmd/symphony`, with `scripts/symphony_runner.mjs` retained as a legacy adapter during migration; use `npm run symphony:report`, `npm run symphony:sync -- --dry-run --json`, `npm run symphony:ui-monitor -- --dry-run`, and `npm run symphony:test` for local queue, dispatcher, and monitor validation.
