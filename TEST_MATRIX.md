...
- `0E` health checks, metrics, and promotion plumbing
  ...
  | `P0-0E-003` | Promotion Gate Requires Named Scenario Passes | Verify promotion tooling/checklist blocks advancement when required scenario evidence is missing. | Confirm staging promotion checklist enforces named scenario completion before main promotion. |
...
- `1D` student invalid-name visibility
  ...
  | `P1-1D-002` | Suggested Corrected Name And Aeries Link | `Site Secretary` | Verify invalid-name details include raw Aeries `FirstName` and `LastName` values, separate normalized corrections for each field, field-level copy actions for those normalized values, and the correct Aeries entry link. | Confirm staging renders the same separate-field suggestion, copy, and link behavior from masked data. |
...
- `3E` bulk summer rollover and recovery
  ...
  | `P3-3E-003` | Fallback Ticket Raised For Unresolved Move Conflict | Verify unresolved move conflicts create sufficient fallback tickets for manual completion. | Confirm staging fallback ticket content is sufficient for IT/manual resolution. |
