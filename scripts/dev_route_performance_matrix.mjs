...
// The self-test exercises planner and strict-merge behavior with synthetic
// artifacts so harness changes can be verified without a live browser session.
async function runRoutePerformanceSelfTest() {
  ...
  try {
    ...
    const failingTransitions = cleanTransitions.map((row) =>
      row.index === 1
        ? syntheticTransitionRow(edgePath, 1, {
            status: "timeout",
            ready: false,
